...
| GS-EGL10-RS-DS-005 | The graphics library shall destroy a surface marked for deletion and invalidate all future references to it when the thread to which the surface is bound makes an otherwise valid eglMakeCurrent call. | | | EGL 1.0 Full Specification, 3.5.4 Destroying Rendering Surfaces, p. 22 | No | Req-EGL-001 | Test |
