...
Verify that eglTerminate succeeds when called again for an already terminated
valid EGLDisplay.
...
/* Terminates an initialized display twice and checks the second result. */
void GS_EGL10_IN_TER_TP_006_init(void) {
    ...
    if (eglTerminate(display) != EGL_TRUE) {
        TEST_LOG_FAIL(test_case, test_procedure,
            "The first eglTerminate call failed, error: 0x%x", eglGetError());
        return;
    }
    ...
    if (result != EGL_TRUE) {
        TEST_LOG_FAIL(test_case, test_procedure,
            "Expected EGL_TRUE for an already terminated display, error: 0x%x",
            error);
        return;
    }
...
/* No drawing is required for this repeated-termination test. */
