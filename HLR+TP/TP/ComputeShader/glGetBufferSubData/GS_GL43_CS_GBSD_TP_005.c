...
/* Covered requirement: GS-GL43-CS-GBSD-006
...
void GS_GL43_CS_GBSD_TP_005_init(void)
{
    ...
    {
        TEST_LOG_FAIL(test_case, test_procedure, "%s (expected 0x%x, got 0x%x, extra 0x%x)",
                      "Expected INVALID_ENUM", (unsigned)GL_INVALID_ENUM, (unsigned)actual_error,
                      (unsigned)extra_error);
        goto finish;
    }
...
int GS_GL43_CS_GBSD_TP_005_result(void) { return test_result; }
