...
/* Covered requirement: GS-GL43-CS-GBSD-005
...
void GS_GL43_CS_GBSD_TP_004_init(void)
{
    ...
    {
        TEST_LOG_FAIL(test_case, test_procedure, "%s (expected 0x%x, got 0x%x, extra 0x%x)",
                      "Decoy buffer setup failed", (unsigned)GL_NO_ERROR, (unsigned)actual_error,
                      (unsigned)extra_error);
        goto finish;
    }
...
int GS_GL43_CS_GBSD_TP_004_result(void) { return test_result; }
