...
/*
 * GS_GLES31_CS_BD_TP_004
 * Covered requirement: GS-GLES31-CS-BD-004.
 * Accept GL_STATIC_READ for the SSBO target and verify GL_BUFFER_USAGE.
 *
 ...
 */
static const char *test_case = "GS_GLES31_CS_BD_TC_004";
static const char *test_procedure = "GS_GLES31_CS_BD_TP_004";
static GLuint buffer;

void GS_GLES31_CS_BD_TP_004_init(void)
{
    ...
    /* Test Case #004: accept GL_STATIC_READ and record it as GL_BUFFER_USAGE. */
    glBufferData(GL_SHADER_STORAGE_BUFFER, size, NULL, GL_STATIC_READ);
    error = glGetError();
    if (error != GL_NO_ERROR)
    {
        TEST_LOG_FAIL(test_case, test_procedure,
                      "glBufferData(GL_STATIC_READ) generated 0x%x", error);
        return;
    }
    glGetBufferParameteriv(GL_SHADER_STORAGE_BUFFER, GL_BUFFER_USAGE, &actual_usage);
    error = glGetError();
    if (error != GL_NO_ERROR || actual_usage != (GLint)GL_STATIC_READ)
    {
        TEST_LOG_FAIL(test_case, test_procedure,
                      "Expected usage 0x%x, got 0x%x; GL error 0x%x",
                      GL_STATIC_READ, (GLuint)actual_usage, error);
        return;
    }
    TEST_LOG_SUCCESS(test_case, test_procedure);
}

void GS_GLES31_CS_BD_TP_004_draw(void) {}

void GS_GLES31_CS_BD_TP_004_close(void)
{
    if (buffer != 0)
    {
